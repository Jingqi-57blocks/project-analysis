...
def test_modules_entrance_links_only_to_the_overview_module_map(tmp_path):
    run = make_run(tmp_path)
    result = generate(run)
    modules = (result.report_dir / "modules.html").read_text(encoding="utf-8")
    assert "System module map" in modules
    assert 'href="doc-project-map.html"' in modules
    assert "Module drill-down" not in modules
