...
def load(module_run: str | Path) -> SourceContext:
    ...
    source_key = "source_run" if manifest.source_mode == "overview-backed" else "source_evidence_run"
    source_value = provenance.get(source_key)
    if not isinstance(source_value, str):
        raise ContractError(f"module provenance has no {source_key} path")
    source = Path(source_value).expanduser().resolve()
    if manifest.source_mode == "overview-backed" and source.name != manifest.source_overview_run:
        raise ContractError("module provenance and source manifest disagree on overview run")
    try:
        source_state = lifecycle.RunState.load(source)
    ...
    problems = source_state.staleness()
