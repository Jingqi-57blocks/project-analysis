...
def _boundary_closure(args) -> int:
    out = write_boundary_closure(load_source_context(args.run))
    _print({"boundary_closure": str(out)})
    return 0
...
def _finalize_model(args) -> int:
    model, audit = finalize_module_model(args.run)
    _print({"module_model": str(model) if model else "", "audit": audit.to_dict()})
...
def run(args) -> int:
    ...
    try:
        handlers = {
            ...
            "module-build-boundary-closure": _boundary_closure,
